...
For a Codex profile with `seat: "luna"`, run `bin/fm-dock.sh resolve --seat luna --harness codex` on the snapshot's host and match exactly one row whose credential home, with a leading `~/` expanded to that host's OS user home, is its resolved physical directory plus `/auth.json`; if no unique row exists, report it as unmeasured rather than using the ambient or default Codex account.
Pass that same directory to `fm-spawn.sh` as `--seat-home` with `--seat luna`, so a changed dock binding refuses the launch instead of starting under a home that was not measured.
Treat a dock resolution failure as a configuration refusal, not a candidate to select around.
