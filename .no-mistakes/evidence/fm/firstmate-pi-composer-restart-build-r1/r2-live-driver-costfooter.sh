...
fm_backend_herdr_send_text_line "$T" "pi --model openai-codex/gpt-5.6-terra" || exit 1
wait_idle || { echo "pi never idle"; screen; exit 1; }
sleep 3
echo "pi=$(pi --version 2>&1|head -1) agent=$(agent)"
echo "### cost-first footer, empty composer (ANSI tail)"; herdr pane read "$PANE" --session "$SESSION" --source visible 2>/dev/null | tail -6 | cat -v
probe "empty composer above cost-first footer"
herdr pane send-text "$PANE" '>' --session "$SESSION" >/dev/null; sleep 1.5
probe "lone '>' above cost-first footer"
control_exit
herdr pane send-keys "$PANE" backspace --session "$SESSION" >/dev/null; sleep 1.5
probe "cleared"
control_exit
