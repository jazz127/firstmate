...
A crewmate creates or updates it lazily through the project's selected delivery path, using `bin/fm-ensure-agents-md.sh` only when the task creates or updates `AGENTS.md`, and preferring pointers to authoritative sources over copied detail.
...
Before steering a lane to touch a pull request branch, verify its forge identity and exclusive ownership, then apply the upstream contribution boundary in [`docs/fork-house-branch.md`](docs/fork-house-branch.md).
