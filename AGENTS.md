...
config/crew-dispatch.json  optional crewmate dispatch profiles; LOCAL, gitignored; firstmate-maintained but human-editable natural-language rules that choose a per-task harness/model/effort/optional-seat profile (section 4). Inherited by secondmate homes
...
config/herdr-presentation-spaces  optional "off" opt-out from, or "on" opt-in to, Herdr's default-on disposable single-task visual projection, which is unconfigured-default-on only at or above a Herdr version floor, or "project" for one workspace per project; LOCAL, gitignored; inherited by secondmate homes; see docs/herdr-backend.md "Presentation spaces"
...
config/ready-session-timeout  optional opt-in to stopping the idle agent of a ship worker whose PR has sat ready awaiting merge for the configured duration (empty = 2 hours), keeping its local copy, branch, PR, and merge monitoring for an on-demand relaunch; LOCAL, gitignored, and not inherited; see docs/configuration.md "Ready-session timeout"
...
state/               runtime records and signals; gitignored
  ...
  <id>.ready-timeout  durable record that the opt-in ready-session timeout stopped (or failed to stop) this ship worker's idle agent while its PR awaited merge; while a `stopped` record names the current spawn_gen and crew state still reads done the worker is deliberately stopped, not stuck, and relaunches on demand; bin/fm-ready-timeout-lib.sh owns it; removed by teardown
...
- `bosun` - load before routing a named Captain's Maneuver upstream or handling its review or merge outcome.
