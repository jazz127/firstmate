...
fm_dod_validate_intent_evidence() {  # <intent> <worktree> <task-temp> [preflight|publish]
  ...
  if printf '%s\n' "$flat_intent" | grep -Eiq '([0-9]+[[:space:]]+of[[:space:]]+[0-9]+[[:space:]]*/[[:space:]]*[0-9]+[[:space:]]+of[[:space:]]+[0-9]+)|(([0-9]+[[:space:]]*(of|/)[[:space:]]*[0-9]+)[^.!?]*(live|verified|real-account|independent|external|externally confirmed))|((live|verified|real-account|independent|external|externally confirmed)[^.!?]*([0-9]+[[:space:]]*(of|/)[[:space:]]*[0-9]+))|((live|verified|real-account|independent|external|externally confirmed)[^.!?]*(evidence|verification|confirmation)[[:space:]]*:)|((live|verified|real-account|independent|external|externally confirmed)[^.!?]*(evidence|verification|confirmation)[^.!?]*(was|were|is|are|shows?|reported|demonstrated|completed|passed|failed|confirmed|verified))|((evidence|verification|confirmation)[^.!?]*(live|verified|real-account|independent|external|externally confirmed)[^.!?]*(was|were|is|are|shows?|reported|demonstrated|completed|passed|failed|confirmed|verified))'; then
    claim=1
  fi
  [ "$claim" -eq 1 ] || return 0
