...
fm_dod_validate_intent_evidence() {  # <intent> <worktree> <task-temp> [preflight|publish]
  ...
  if printf '%s\n' "$flat_intent" | grep -Eiq '([0-9]+[[:space:]]+of[[:space:]]+[0-9]+[[:space:]]*/[[:space:]]*[0-9]+[[:space:]]+of[[:space:]]+[0-9]+)|(([0-9]+[[:space:]]*(of|/)[[:space:]]*[0-9]+)[^.!?]*(live|verified|real-account|independent|external|externally confirmed))|((live|verified|real-account|independent|external|externally confirmed)[^.!?]*([0-9]+[[:space:]]*(of|/)[[:space:]]*[0-9]+))|((live|verified|real-account|independent|external|externally confirmed)[^.!?]*(evidence|verification|confirmation|assertion|claim))|((evidence|verification|confirmation|assertion|claim)[^.!?]*(live|verified|real-account|independent|external|externally confirmed))'; then
    claim=1
  fi
  [ "$claim" -eq 1 ] || return 0
