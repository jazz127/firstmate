...
handle_paused_stale() {  # <window> <task> <hash>
  ...
  if done_open_pr_wait "$task" "$(last_status_line "$STATE/$task.status")"; then
...
EOF
  ...
  while IFS= read -r w; do
    ...
    key=$(window_key "$w")
    last=$(status_declared_wait_line "$STATE/$task.status")
    if ! status_is_paused_or_captain_held "$last" && ! done_open_pr_wait "$task" "$(last_status_line "$STATE/$task.status")" \
    ...
    if [ "$h" = "$prev" ]; then
      ...
      if [ "$n" -ge 2 ] && [ "$busy_now" -ne 0 ]; then
        ...
        if [ "$kind" = secondmate ]; then
        ...
        elif done_open_pr_wait "$task" "$(last_status_line "$STATE/$task.status")" && ! task_captain_call_open "$task"; then
          case "$(pause_state_class "$w" "$task")" in
          ...
          if [ "$(cat "$sf" 2>/dev/null || true)" != "$h" ]; then
            ...
            if [ -e "$pf" ] || status_is_paused_or_captain_held "$(status_declared_wait_line "$STATE/$task.status")" \
              || done_open_pr_wait "$task" "$(last_status_line "$STATE/$task.status")"; then
              case "$(pause_state_class "$w" "$task")" in
                paused)  handle_paused_stale "$w" "$task" "$h" ;;
        ...
        if [ "$paused_bound" -ne 0 ] && [ -e "$pf" ] && { [ "$n" -ge 2 ] \
          || { ! status_is_paused_or_captain_held "$last" && ! done_open_pr_wait "$task" "$(last_status_line "$STATE/$task.status")"; }; }; then
          clear_pause_tracking "$key"
        fi
      fi
    else
      ...
      task=$(window_to_task "$w" "$STATE")
      if ! afk_present && { status_is_paused_or_captain_held "$last" || done_open_pr_wait "$task" "$(last_status_line "$STATE/$task.status")"; } && [ "$busy_now" -ne 0 ]; then
        case "$(pause_state_class "$w" "$task")" in
          paused) handle_paused_stale "$w" "$task" "$h" ;;
