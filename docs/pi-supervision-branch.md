...
A `routine` outcome explicitly reported with `silent=true` is stored as already handled and delivered with no rendered note and no wake of main, while every other `routine` outcome stays rendered with its sailboat prefix.
