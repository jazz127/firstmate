...
A routine outcome explicitly reported with `silent=true` (a no-change heartbeat or pure already-recorded bookkeeping, per [docs/pi-supervision-branch.md](../pi-supervision-branch.md#two-stage-noise-filter)) is delivered silently with no rendered note, while every other routine outcome returns as an appended, rendered note that leads with ⛵ then the dim outcome text.
