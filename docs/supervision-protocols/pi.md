...
Routine outcomes remain hidden and turn-free under the presentation contract, independently of Calm and the `silent` marker; do not turn them into unsolicited chat updates or shipshape replies. `silent=true` is legal on any routine row when it only restates durably recorded bookkeeping, and is excluded from the task coverage index.
