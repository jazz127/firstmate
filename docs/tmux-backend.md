...
The shared classifier accepts a shell glyph as an empty agent composer only inside a bordered container, or as Pi's own first-row `>` inside an identity-proven Pi separator pair when the adapter opts in; tmux never does, so a lone first-row `>` there stays input ([Pi composer states](herdr-backend.md#pi-composer-states)).
