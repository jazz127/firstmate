...
The shared classifier accepts a shell glyph as an empty agent composer only inside a bordered container, or as Pi's own first-row `>` inside an identity-proven Pi separator pair.
