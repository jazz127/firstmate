...
It confirms the handling handoff against that successor before scheduling the follow-up and retries once against the current generation and successor.
If confirmation is still rejected, Pi retires that arm and restores a successor within its bounded retry limit before delivering the original wake; an exhausted rejection or failed retirement surfaces one typed failure.
...
It also forces a handling confirmation rejection through a mismatched watcher PID, checks restoration before one follow-up while the wake row stays durable, and checks that a permanent rejection remains a bounded typed failure.
