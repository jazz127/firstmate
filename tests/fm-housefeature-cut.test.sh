...
test_classify_skips() {
  ...
  assert_equals "cut reconcile-inactive-crew-investigate-s1" \
    "$(classify_with HF_HEAD_REF=fm/reconcile-inactive-crew-investigate-s1)" "ordinary reconcile feature is cut"
...
test_run_cuts_and_captures() {
  ...
  out=$(run_cut fm/firstmate-house-upstream-merge-r1 "$head" "$merge" HF_PR_NUMBER=9) || fail "skip run failed"
  assert_contains "$out" "Skipped pull request #9" "skip is reported"
  remote_ref housefeature/house-upstream-merge >/dev/null && fail "skip must not push"
...
test_run_is_create_only_and_reports_failures() {
  local shas head merge out rc
  shas=$(merge_feature fm/firstmate-delta-c2 house.txt delta)
  head=${shas% *} merge=${shas#* }
  git clone -q --mirror "$TMP_ROOT/fork.git" "$TMP_ROOT/stale.git"
  git --git-dir="$TMP_ROOT/fork.git" update-ref refs/heads/housefeature/delta "$head^"
  git -C "$TMP_ROOT/clone" remote add racer "$TMP_ROOT/stale.git"
  git -C "$TMP_ROOT/clone" remote set-url --push racer "$TMP_ROOT/fork.git"
  rc=0
  out=$(run_cut fm/firstmate-delta-c2 "$head" "$merge" HF_REMOTE=racer 2>&1) || rc=$?
  expect_code 1 "$rc" "push onto a branch created after the check fails"
  assert_contains "$out" "was refused" "refused push is reported"
  assert_equals "$(git -C "$TMP_ROOT/clone" rev-parse "$head^")" "$(remote_ref housefeature/delta)" \
    "branch created after the check is never fast-forwarded"

  rc=0
  out=$(run_cut fm/firstmate-epsilon-r1 "$head" 0000000000000000000000000000000000000001 2>&1) || rc=$?
  expect_code 1 "$rc" "missing merge commit fails"
  assert_grep "Could not cut \`housefeature/epsilon\` for pull request #7" "$TMP_ROOT/summary.md" "failure is summarized"
  remote_ref housefeature/epsilon >/dev/null && fail "failed run must not push"
  pass "push is create-only and failed runs write a summary line"
}
